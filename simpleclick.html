...
for( var r in byrc ) {
 for( var c in byrc[r] ) {
  byrc[r][c].jQ.click( ( function(myr,myc) {
   return function() {
    var i=rc2i[myr+','+myc];
    byrc[0][0].command_nolatency( rc2i[myr+','+myc], [255,0,0], 0, 200 );
    for( var a=0; a<3; a++ ) {
     for( var b=0; b<3; b++ ) {
      byrc[0][0].command( rc2i[(parseInt(myr)+a-1)+','+(parseInt(myc)+b-1)], [150,0,80], 0, 200 );
     }
    }
   };
  } )( r,c ) );
